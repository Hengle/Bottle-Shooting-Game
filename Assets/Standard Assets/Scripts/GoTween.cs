...
public class GoTween : AbstractGoTween
{
	...
	public override bool update(float deltaTime)
	{
		...
		{
			UnityEngine.Debug.LogWarning("target validation failed. destroying the tween to avoid errors. Target type: " + targetType);
			base.autoRemoveOnComplete = true;
			return true;
		}
		if (!_didBegin)
		...
		float totalElapsedTime = (!_isLoopingBackOnPingPong) ? _elapsedTime : (base.duration - _elapsedTime);
		for (int i = 0; i < _tweenPropertyList.Count; i++)
		{
			_tweenPropertyList[i].tick(totalElapsedTime);
		}
		onUpdate();
