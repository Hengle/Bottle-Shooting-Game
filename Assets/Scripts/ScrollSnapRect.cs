...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IEventSystemHandler
{
	...
	private void Start()
	{
		...
		if ((Global.currentLevel > 6 && Global.currentLevel < 13) || (Global.currentLevel > 18 && Global.currentLevel < 25) || (Global.currentLevel > 30 && Global.currentLevel < 37))
		{
			NextScreen();
		}
		if ((Global.currentLevel > 111 && Global.currentLevel < 119) || (Global.currentLevel > 211 && Global.currentLevel < 219))
		{
			NextScreen();
			NextScreen();
		}
		if ((Global.currentLevel > 118 && Global.currentLevel < 125) || (Global.currentLevel > 218 && Global.currentLevel < 225))
		{
			NextScreen();
			NextScreen();
			NextScreen();
	...
	private void SetPagePositions()
	{
		...
		for (int i = 0; i < _pageCount; i++)
		{
			...
			Vector2 a = component.anchoredPosition = ((!_horizontal) ? new Vector2(0f, -(i * num2 - num6 / 2 + num4)) : new Vector2(i * num - num5 / 2 + num3, 0f));
			_pagePositions.Add(-a);
		}
	}
	...
	private void InitPageSelection()
	{
		...
		for (int i = 0; i < pageSelectionIcons.childCount; i++)
		{
			...
			if (component == null)
			{
				UnityEngine.Debug.LogWarning("Page selection icon at position " + i + " is missing Image component");
			}
	...
	private void NextScreen()
	{
		...
		if (Global.world == "N" || Global.world == "D")
		{
			if (_currentPage == 3)
			{
		...
		prevButton.SetActive(value: true);
	}
	...
	public void OnEndDrag(PointerEventData aEventData)
	{
		...
		if (_horizontal)
		{
			...
			Vector2 anchoredPosition = _container.anchoredPosition;
			num = x - anchoredPosition.x;
		}
		else
		{
			float y = _startPosition.y;
			Vector2 anchoredPosition2 = _container.anchoredPosition;
			num = 0f - (y - anchoredPosition2.y);
		}
