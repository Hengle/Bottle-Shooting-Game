	...
	public class ClipperOffset
	{
		...
		private void OffsetPoint(int j, ref int k, JoinType jointype)
		{
			...
			if (m_sinA * m_delta < 0.0)
			{
				...
				double num2 = intPoint.X;
				DoublePoint doublePoint5 = m_normals[k];
				long x3 = Round(num2 + doublePoint5.X * m_delta);
				IntPoint intPoint2 = m_srcPoly[j];
				double num3 = intPoint2.Y;
				DoublePoint doublePoint6 = m_normals[k];
				destPoly.Add(new IntPoint(x3, Round(num3 + doublePoint6.Y * m_delta)));
				m_destPoly.Add(m_srcPoly[j]);
				List<IntPoint> destPoly2 = m_destPoly;
				IntPoint intPoint3 = m_srcPoly[j];
				double num4 = intPoint3.X;
				DoublePoint doublePoint7 = m_normals[j];
				long x4 = Round(num4 + doublePoint7.X * m_delta);
				IntPoint intPoint4 = m_srcPoly[j];
				double num5 = intPoint4.Y;
				DoublePoint doublePoint8 = m_normals[j];
				destPoly2.Add(new IntPoint(x4, Round(num5 + doublePoint8.Y * m_delta)));
			}
			else
			{
				switch (jointype)
				...
				case JoinType.jtMiter:
				{
					...
					double x5 = doublePoint9.X;
					DoublePoint doublePoint10 = m_normals[k];
					double num6 = x5 * doublePoint10.X;
					DoublePoint doublePoint11 = m_normals[j];
					double y = doublePoint11.Y;
					DoublePoint doublePoint12 = m_normals[k];
					double num7 = 1.0 + (num6 + y * doublePoint12.Y);
					if (num7 >= m_miterLim)
					{
						DoMiter(j, k, num7);
					}
		...
		internal void DoRound(int j, int k)
		{
			...
			int num2 = (int)Round(m_StepsPerRad * Math.Abs(value));
			DoublePoint doublePoint5 = m_normals[k];
			...
			List<IntPoint> destPoly2 = m_destPoly;
			IntPoint intPoint3 = m_srcPoly[j];
			double num6 = intPoint3.X;
			DoublePoint doublePoint7 = m_normals[j];
			long x3 = Round(num6 + doublePoint7.X * m_delta);
			IntPoint intPoint4 = m_srcPoly[j];
			double num7 = intPoint4.Y;
			DoublePoint doublePoint8 = m_normals[j];
